jQuery(document).ready(function($) {
	...
	jQuery(".add-field-mapping").on('click', function(){
		...
		$('#addrow').val(newrow_value);
                jQuery.ajax({
                        ...
                        url: ajaxurl,
                        data: { 
				'action': 'convpfm_fieldmapping_dropdown', 
				'rowCount': rowCount, 
				'security': nonce,
			...
			$( '#convpfm-fieldmapping-table' ).append('<tr style="margin: 5px 0px 0 5px;"><td class="text-center p-1"><input type="hidden" name="attributes[' + rowCount + '][rowCount]" value="' + rowCount + '"><input type="checkbox" name="record" class="checkbox-field "></td><td class="p-1 text-start"><select name="attributes[' + rowCount + '][attribute]" class="select-field feed-attr">' + data.field_options + '</select></td><td class="p-1 text-start"><input type="text" name="attributes[' + rowCount + '][prefix]" class="input-field-medium attr-prefix"></td><td class="p-1 text-start"><select name="attributes[' + rowCount + '][mapfrom]" class="select-field feed-attr">' + data.attribute_options + '</select></td><td class="p-1 text-start"><input type="text" name="attributes[' + rowCount + '][suffix]" class="input-field-medium attr-prefix"></td></tr>');

			$('.select-field').on('change', function(){
				if ($(this).val() == "static_value") {
					var rownr = $(this).closest("tr").prevAll("tr").length;
					$(this).replaceWith('<input type="text" name="attributes[' + rowCount + '][mapfrom]" class="input-field-midsmall" style="width: 100%;min-height: 38px"><input type="hidden" name="attributes[' + rowCount + '][static_value]" value="true">');
				}
			});
                })
                .fail(function( data ) {
	...
	jQuery(".add-own-mapping").on('click', function(){
		...
		$('#addrow').val(newrow_value);
                ...
                .done(function( data ) {
                        data = JSON.parse( data );
			$( '#convpfm-fieldmapping-table' ).append('<tr style="margin: 5px 0px 0 5px;"><td class="p-1 text-center"><input type="hidden" name="attributes[' + rowCount + '][rowCount]" value="' + rowCount + '"><input type="checkbox" name="record" class="checkbox-field"></td><td class="p-1 text-start"><input name="attributes[' + rowCount + '][attribute]" id="own-input-field" class="input-field feed-attr"></td><td class="p-1 text-start"><input type="text" name="attributes[' + rowCount + '][prefix]" class="input-field-medium attr-prefix"></td><td class="p-1 text-start"><select name="attributes[' + rowCount + '][mapfrom]" class="select-field feed-attr">' + data.attribute_options + '</select></td><td class="p-1 text-start"><input type="text" name="attributes[' + rowCount + '][suffix]" class="input-field-medium attr-prefix"></td></tr>');
...
//			var rownr = $(this).closest("tr").prevAll("tr").length;
			...
			$(this).replaceWith('<input type="text" name="attributes[' + rownr + '][mapfrom]" class="input-field-midsmall" style="width:100%; min-height: 38px"><input type="hidden" name="attributes[' + rownr + '][static_value]" value="true">');
		}
	});
