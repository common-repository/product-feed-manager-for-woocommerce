    ...
    if (variations == '1') {
      jQuery
        .ajax({
          method: 'POST',
          url: ajaxurl,
          data: { action: 'convpfm_shipping_zones' },
        })
        .done(function (data) {
  ...
  jQuery('#countries').on('change', function () {
    ...
    jQuery
      .ajax({
        method: 'POST',
        url: ajaxurl,
        data: { action: 'convpfm_channel', country: country, security: security },
      })
  ...
  jQuery('#fileformat').on('change', function () {
    ...
    if (fileformat == 'xml') {
      ...
      if ($('#delimiter').length == 0) {
        $('#file').after(
          '<tr id="delimiter"><td><span class="convpfm-label">Delimiter:</span></td><td><select style="width:128px" id="delimiter_val" name="delimiter" class="select-field"><option value=",">, comma</option><option value="|">| pipe</option><option value=";">;</option><option value="tab">tab</option><option value="#">#</option></select></td></tr>'
        );
