...
"5644 - Vehicles & Parts > Vehicles > Watercraft > Yachts",
		...
		var rows = $('#convpfm-ajax-mapping-table tbody tr.catmapping').length;
		var $tr = $(this).closest('tr');
		var fromRow = $('#convpfm-ajax-mapping-table tr').index($tr)-1;
		...
		jQuery.ajax({
                	method: "POST",
                        url: ajaxurl,
                       	data: { 
				'action': 'convpfm_add_mass_cat_mapping', 
				'security': nonce,
			...
			if (len >= minimum){
				if ( !isNaN(parseInt(map_to_category)) ) {
					jQuery.ajax({
                        			...
                        			url: ajaxurl,
                       	 			data: { 
							'action': 'convpfm_add_cat_mapping', 
							'rowCount': rowCount, 
