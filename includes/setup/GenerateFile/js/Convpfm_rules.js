jQuery(document).ready(function ($) {
  ...
  jQuery(".add-field-manipulation").on("click", function () {
    ...
    var TrueRowCount = $("#convpfm-ajax-table >tbody >tr").length - 1;
    ...
    jQuery
      ...
      .done(function (data) {
        ...
        if (TrueRowCount == 0) {
          $("#convpfm-ajax-table")
            ...
            .append(
              '<tr><td valign="top"><input type="hidden" name="field_manipulation[' +
                ...
                data.rowCount +
                '][product_type]" class="select-field"><option value="all">Simple and variable</option><option value="simple">Simple</option><option value="variable">Variable</option></select></td></td><td valign="top"><select name="field_manipulation[' +
                data.rowCount +
                '][attribute]" id="field_manipulation_' +
                data.rowCount +
                '">' +
                data.dropdown +
                '</select></td><td class="becomes_fields_' +
                data.rowCount +
                '" valign="top"><select name="field_manipulation[' +
                data.rowCount +
                '][becomes][1][attribute]" id="field_manipulation_becomes_attribute_' +
                data.rowCount +
                '">' +
                data.dropdown +
                '</select></td><td><span class="dashicons dashicons-plus field_extra field_manipulation_extra_' +
                data.rowCount +
          ...
          $(
            '<tr><td valign="top"><input type="hidden" name="field_manipulation[' +
              ...
              '" valign="top"><select name="field_manipulation[' +
              data.rowCount +
              '][becomes][1][attribute]" id="field_manipulation_becomes_' +
              data.rowCount +
              '">' +
              data.dropdown +
              '</select><td><span class="dashicons dashicons-plus field_extra field_manipulation_extra_' +
              data.rowCount +
              '" style="display: inline-block;" title="Add an attribute to this field"></span></td></tr>'
        ...
        jQuery(".field_manipulation_extra_" + rowCount).on(
          "click",
          function () {
            plusCount = Math.round(new Date().getTime() + Math.random() * 100);
            jQuery(".becomes_fields_" + rowCount).append(
              '<br/><select name="field_manipulation[' +
                data.rowCount +
                "][becomes][" +
                plusCount +
                '][attribute]" id="field_manipulation_becomes_attribute_' +
                data.rowCount +
                '">' +
                data.dropdown +
                "</select>"
      ...
      .done(function (data) {
        ...
        if (TrueRowCount == 0) {
          $("#convpfm-ajax-table")
            .find("tbody:first")
            .append(
              '<tr style="margin: 5px 0px 0 5px;" class="convpfm-filter-row"><td class="text-center p-1"><input type="hidden" name="rules[' +
                data.rowCount +
                '][rowCount]" value="' +
                data.rowCount +
                '"><input type="checkbox" name="record" class="checkbox-field"></td><td class="p-1 text-start"><select  class="filter-option" name="rules[' +
                data.rowCount +
                '][attribute]" id="rules_' +
                data.rowCount +
                '">' +
                data.dropdown +
                '</select></td><td class="p-1 text-start"><select class="filter-condition" name="rules[' +
                data.rowCount +
                '][condition]" class="select-field"><option value="contains">contains</option><option value="containsnot">does not contain</option><option value="=">is equal to</option><option value="!=">is not equal to</option><option value=">">is greater than</option><option value=">=">is greater or equal to</option><option value="<">is less than</option><option value="=<">is less or equal to</option><option value="empty">is empty</option><option value="notempty">is not empty</option></select></td><td class="p-1 text-start"><input class="filter-value" type="text" name="rules[' +
                rowCount +
                '][criteria]" class="input-field-large" id="criteria_' +
                data.rowCount +
                '"></td><td class="p-1 text-start"><select class="filter-value" name="rules[' +
                rowCount +
                '][than]" class="select-field filter-then"><optgroup label="Action">Action:<option value="exclude"> Exclude</option><option value="include_only">Include only</option></optgroup></select></td><td>&nbsp;</td></tr>'
            );
        } else {
          $(
            '<tr style="margin: 5px 0px 0 5px;" class="convpfm-filter-row"><td class="text-center p-1"><input type="hidden" name="rules[' +
        ...
        jQuery("#rules_" + rowCount).on("change", function () {
          if ($(this).val() == "categories") {            
            ...
            if (checkNumeric) {
              jQuery
                .ajax({
                  ...
                  url: ajaxurl,
                  data: {
                    action: "convpfm_categories_dropdown",
                    rowCount: rowCount,
                  },
                })
                ...
                .done(function (data) {
                  data = JSON.parse(data);
                  jQuery("#criteria_" + rowCount).replaceWith(
                    "" + data.dropdown + ""
                  );
            ...
            jQuery("#criteria_" + rowCount).replaceWith(
              '<input class="filter-value" type="text" name="rules[' +
                rowCount +
                '][criteria]" class="input-field-large" id="criteria_' +
                rowCount +
                '">'
          ...
          $("#convpfm-ajax-table")
            ...
            .append(
              '<tr><td><input type="hidden" name="rules2[' +
                ...
                data.rowCount +
                '][attribute]" class="select-field">' +
                data.dropdown +
                '</select></td><td><select name="rules2[' +
                data.rowCount +
                '][condition]" class="select-field"  id="condition_' +
                data.rowCount +
                ...
                rowCount +
                '][criteria]" class="input-field-large"></td><td><select name="rules2[' +
                data.rowCount +
                '][than_attribute]" class="select-field" id="than_attribute_' +
                rowCount +
                '" style="width:150px;">' +
                data.dropdown +
            ...
            jQuery(".js-autosuggest").on("click", function () {
              ...
              jQuery(this).keyup(function () {
                ...
                if (len >= minimum) {
                  jQuery(this)
                    .closest("input")
                    .removeClass("input-field-large");
                  jQuery(this)
                    .closest("input")
                    .addClass("input-field-large-active");
                } else {
                  jQuery(this)
                    .closest("input")
                    .removeClass("input-field-large-active");
