...
class Convpfm_UpdateProject {
     ...
     */
        ...
        $convpfm_channels = get_option( 'convpfm_channels' );

        foreach ( $convpfm_channels as $key => $val ) {
    ...
    public static function get_project_data( $project_hash ) {
        if ( get_option( 'convpfm_cron_files' ) ) {
    ...
    public static function update_project( $project_data ) {
        ...
        if ( ! array_key_exists( 'project_hash', $project_data ) ) {
                    ...
                    $external_path = $external_base . '/conversios-product-feed/' . $project_data['fileformat'];
            $convpfm_channels       = get_option( 'convpfm_channels' );
            ...
            foreach ( $convpfm_channels as $key => $val ) {
                ...
                foreach ( $val as $k => $v ) {
                    if ( $project_data['channel_hash'] == $v['channel_hash'] ) {
                        ...
                        $project_fill['query_log']        = $external_base . '/conversios-product-feed/logs/query.log';
                        $project_fill['query_output_log'] = $external_base . '/conversios-product-feed/logs/query_output.log';
                    }
                }
            }
                    update_option( 'convpfm_channel_files', $project_fill, '' );
        } else {
    ...
    public static function reconfigure_project( $project_data ) {
        ...
        $upload_dir                      = wp_upload_dir();
                $external_base           = $upload_dir['baseurl'];
                $external_path           = $external_base . '/conversios-product-feed/' . $project_config['fileformat'];
         ...
         */
                if ( ( array_key_exists( 'convpfm_page', $project_data ) ) && ( $project_data['convpfm_page'] == 'filters_rules' ) ) {
         ...
         */
                if ( ( array_key_exists( 'convpfm_page', $project_data ) ) && ( $project_data['convpfm_page'] == 'field_manipulation' ) ) {
    ...
    public static function add_project_cron( $project_data, $key ) {
        if ( get_option( 'convpfm_cron_files' ) ) {
            ...
            update_option( 'convpfm_cron_files', $cron_projects );
        } else {
