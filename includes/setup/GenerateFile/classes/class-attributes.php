...
class Convpfm_Attributes
{
	...
	public function get_channels($country)
	{
		$channels = array();
		$channel_configs = get_option('convpfm_channels');
		$excluded_channels = array("Google Shopping","Meta / Facebook Catalog Feed / Instagram", "Google Merchant Promotions Feed", "Google Remarketing - DRM", "Google - DSA", "Google Local Products", "Google Local Products Inventory", "Google Product Review", "TikTok Product Catalog");
		$excluded_countries = array("Argentina", "Belgium", "Brazil","Bulgaria","Chile","Czech Republic","Denmark","Finland","Greece","Hungary","Ireland","Israel","Japan","Latvia","Mexico","Norway","Poland","Portugal","Romania","Russian Federation","Serbia","Slovakia","South Africa","Spain","Sweden","Switzerland","Ukraine", );

		// Function to filter channels based on excluded channels
		$filter_channels = function ($channel) use ($excluded_channels) {
			return !in_array($channel['name'], $excluded_channels);
		};
		if(!empty($channel_configs)) {
			// Get general channels (Custom Feed and All countries)
			foreach ($channel_configs as $key => $val) {
				if ($key == "Custom Feed" || $key == "All countries") {
					// Filter out excluded channels
					$filtered_channels = array_filter($val, $filter_channels);
					$channels = array_merge($channels, $filtered_channels);
				}
			}
		}		
		if(!empty($country)) {
			// Handle excluded countries
			if (!in_array($country, $excluded_countries)) {
				// Get country-specific channels
				if(!empty($channel_configs)) {
					foreach ($channel_configs as $key => $val) {
						if (preg_match("/-$country/i", $key) || $country == $key) {
							// Filter out excluded channels
							$filtered_channels = array_filter($val, $filter_channels);
							$channels = array_merge($channels, $filtered_channels);
						}
	...
	private function get_custom_attributes()
	{
		...
		if (!function_exists('convpfm_get_meta_keys_for_post_type')) :

			function convpfm_get_meta_keys_for_post_type($post_type, $sample_size = 'modified')
			{
				$meta_keys = array();

				$add_convpfm_basic = get_option('add_convpfm_basic');
				if ($add_convpfm_basic == "yes") {
		...
		$post_type = "product";
		$data = convpfm_get_meta_keys_for_post_type($post_type);
	...
	public function get_product_attributes_dropdown()
	{
		...
		if (get_option('convpfm_extra_attributes')) {
			...
			if ($extra_attributes) {
				array_walk($extra_attributes, function (&$value, $key) {
					$value .= ' (Added Custom attribute)';
				});
	...
	public function get_filter_product_attributes(){
		$sitename = get_option('blogname');
		$attributes = array(
			"id" => "Product Id",
			"categories" => "Category",
	...
	public function get_filter_product_attributes_array()
	{
		$sitename = get_option('blogname');
		$attributes = array(
			"id" => "Product Id",
	...
	public function get_product_attributes()
	{
		...
		if (is_array($this->get_dynamic_attributes())) {
			$dynamic_attributes = $this->get_dynamic_attributes();
			array_walk($dynamic_attributes, function (&$value, $key) {
		...
		if (is_array($custom_attributes)) {
			array_walk($custom_attributes, function (&$value, $key) {
