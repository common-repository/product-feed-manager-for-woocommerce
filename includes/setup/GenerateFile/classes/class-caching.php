...
class Convpfm_Caching {
	...
	/**
		...
		$litespeed_ex_paths = maybe_unserialize( get_option( 'litespeed.conf.cdn-exc' ) );
               	if ( $litespeed_ex_paths && is_array( $litespeed_ex_paths ) && ! in_array( '/wp-content/uploads/conversios-product-feed', $litespeed_ex_paths ) ) {
                       	$litespeed_ex_paths = array_merge(
                               	$litespeed_ex_paths,
                               	array( '/wp-content/uploads/conversios-product-feed' )
                       	);
		...
		$wp_fastest_cache_ex_paths = json_decode( get_option( 'WpFastestCacheExclude' ), false );
			...
			$feed_path_exist = false;
                        ...
                        foreach ( $wp_fastest_cache_ex_paths as $path ) {
                                if ( 'product-feed-manager-for-woocommerce' === $path->content ) {
                        ...
                        if ( ! $feed_path_exist ) {
                                ...
                                $new_rule->content = 'product-feed-manager-for-woocommerce';
                                $new_rule->type    = "page";
        ...
        public function wp_super_cache() {
                ...
                if ( $wp_super_ex_paths && strpos( $wp_super_ex_paths, 'product-feed-manager-for-woocommerce' ) === false ) {
                        $wp_super_ex_paths = explode( ',', $wp_super_ex_paths );
                        $wp_super_ex_paths = array_merge( $wp_super_ex_paths, [ 'product-feed-manager-for-woocommerce' ] );
        ...
        public function wp_optimize_cache() {
                ...
                if ( isset( $wp_optimize_ex_paths['enable_page_caching'] ) && $wp_optimize_ex_paths['enable_page_caching'] && is_array( $wp_optimize_ex_paths ) && ! in_array( '/wp-content/uploads/conversios-product-feed', $wp_optimize_ex_paths['cache_exception_urls'], true ) ) {
                        $woo_feed_ex_path['cache_exception_urls'] = [ '/wp-content/uploads/conversios-product-feed' ];
        ...
        public function cache_enabler_cache() {
                ...
                if ( isset( $cache_enabler_ex_paths['excluded_page_paths'] ) && empty( $cache_enabler_ex_paths['excluded_page_paths'] ) ) {
                        $cache_enabler_ex_paths['excluded_page_paths'] = '/wp-content/uploads/conversios-product-feed/';
        ...
        public function swift_performance_cache() {
                ...
                if ( $swift_perform_ex_paths && isset( $swift_perform_ex_paths['exclude-strings'] ) ) {
                        ...
                        if ( is_array( $exclude_strings ) && ! in_array( '/wp-content/uploads/conversios-product-feed', $exclude_strings, true ) ) {
                                $woo_feed_ex_path['exclude-strings'] = [ '/wp-content/uploads/conversios-product-feed' ];
                                $swift_perform_ex_paths              = array_merge_recursive(
                                        $swift_perform_ex_paths,
                                        $woo_feed_ex_path
                                );
                        } else {
                                $swift_perform_ex_paths['exclude-strings'] = [ '/wp-content/uploads/conversios-product-feed' ];
                        }
        ...
        public function comet_cache() {
                ...
                if ( $comet_cache_settings && isset( $comet_cache_settings['exclude_uris'] ) ) {
                        ...
                        if ( strpos( $exclude_uris, '/wp-content/uploads/conversios-product-feed' ) === false ) {
                                $exclude_uris                         .= "\n/wp-content/uploads/conversios-product-feed";
        ...
        public function hyper_cache() {
                ...
                if ( $hyper_cache_settings && isset( $hyper_cache_settings['reject_uris'] ) ) {
                        ...
                        if ( is_array( $exclude_strings ) && ! in_array( '/wp-content/uploads/conversios-product-feed', $exclude_strings, true ) ) {
                                $woo_feed_ex_path['reject_uris']         = [ '/wp-content/uploads/conversios-product-feed' ];
