<?php
if ( ! class_exists( 'Convpfm_Survey' ) ) {	
	class Convpfm_Survey {
		public $name;
		public $plugin;
		...
		public function __construct( $name = '', $plugin = '' ){
			...
			$this->name   = $name;
			$this->plugin = $plugin;
			$this->TVC_Admin_Helper = new Convpfm_TVC_Admin_Helper();
		...
		public function tvc_js() {
			...
			jQuery(function($){
				...
				$form.submit(function(event) {
					...
					var data = {
						action:'convpfm_call_add_survey',
			...
			<div class="ee-survey-modal" id="ee-survey-<?php echo esc_html($this->plugin); ?>">
				<div class="ee-survey-wrap">
					<form class="ee-survey" method="post">
						<span class="ee-survey-title"><?php echo ' ' . esc_html__( 'We appreciate your feedback on how we can improve.', 'product-feed-manager-for-woocommerce' ); ?></span>
						<div class="feedback-reasons">
							<div tabindex="0" class="feedback-reason" onclick="selectReason(this)" data-question="<?php esc_html_e('Which plugin are you considering?', 'product-feed-manager-for-woocommerce') ?>">
								<span class="dashicons dashicons-search"></span>
								<p><?php esc_html_e('Found a better Plugin', 'product-feed-manager-for-woocommerce') ?></p>
							</div>
							<div tabindex="0" class="feedback-reason" onclick="selectReason(this)" data-question='<?php esc_html_e("Could you specify the product limitation you're facing?", 'product-feed-manager-for-woocommerce') ?>'>
								<span class="dashicons dashicons-products"></span>
								<p><?php esc_html_e('Product Limit', 'product-feed-manager-for-woocommerce') ?></p>
							</div>
							<div tabindex="0" class="feedback-reason" onclick="selectReason(this)" data-question="<?php esc_html_e('Would you like us to assist you?', 'product-feed-manager-for-woocommerce') ?>">
								<span class="dashicons dashicons-editor-help"></span>
								<p><?php esc_html_e("Couldn't Understand", 'product-feed-manager-for-woocommerce') ?></p>
							</div>
							<div tabindex="0" class="feedback-reason" onclick="selectReason(this)" data-question="<?php esc_html_e('Could you tell us more about the feature?', 'product-feed-manager-for-woocommerce') ?>">
								<span class="dashicons dashicons-admin-generic"></span>
								<p><?php esc_html_e('Missing a Specific Feature', 'product-feed-manager-for-woocommerce') ?></p>
							</div>
							<div tabindex="0" class="feedback-reason" onclick="selectReason(this)" data-question="<?php esc_html_e('Can you describe the issue?', 'product-feed-manager-for-woocommerce') ?>">
								<span class="dashicons dashicons-warning"></span>
								<p><?php esc_html_e('Bugs', 'product-feed-manager-for-woocommerce') ?></p>
							</div>
						</div>
						...
						<p class="description text-danger mt-3"><span style="color:red">*</span><?php esc_html_e("Please, select one reason and submit.", 'product-feed-manager-for-woocommerce') ?></p>
						<div class="feedback-footer">
							...
							<button type="submit" class="button submit button-primary">Submit & Deactivate</button>
						</div>
